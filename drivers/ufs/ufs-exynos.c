...
static int exynos_ufs_phy_init(struct exynos_ufs *ufs)
{
	...
	ret = generic_phy_get_by_name(dev, "ufs-phy", &phy);
	if (ret) {
...
static int exynos_ufs_parse_dt(struct udevice *dev, struct exynos_ufs *ufs)
{
	...
	if (IS_ERR(ufs->sysreg))
		...
		if (dev_read_u32(dev, "samsung,sysreg", &ufs->iocc_offset)) {
			dev_warn(
				dev,
...
static const struct udevice_id exynos_ufs_of_match[] = {
	{ .compatible = "google,gs101-ufs",
	  .data	      = (ulong)&gs101_ufs_drvs },
	{ .compatible = "samsung,exynos7-ufs",
	  .data	      = (ulong)&exynos_ufs_drvs },
	{ .compatible = "samsung,exynosautov9-ufs",
	  .data	      = (ulong)&exynosauto_ufs_drvs },
#ifdef EXYNOSAUTO_UFS_VH
	{ .compatible = "samsung,exynosautov9-ufs-vh",
	  .data	      = (ulong)&exynosauto_ufs_vh_drvs },
#endif
	{ .compatible = "tesla,fsd-ufs",
	  .data       = (ulong)&fsd_ufs_drvs },
	{},
};
